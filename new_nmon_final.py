...
def create_document_with_images(metrics, image_dir, output_dir, document_name, hostname, date, image_height=3, image_width=7, main_heading="NMON Analysis Report"):
    ...
    Args:
    - metrics (list): List of metrics to look for in the image filenames.
...
def send_email(host_dirs, body,  out_dir):
    ...
    msg = MIMEMultipart()
    msg['From'] = 'sender mail'  # Change as needed
...
def concat_documents_with_styles(top_file, middle_file, bottom_file, output_file):
    """
    Merges the top, middle, and bottom documents into a single document, ensuring headers, footers, and images are preserved.
    """
    # Load the documents
    ...
    # Append middle and bottom content into the top document
    for src_doc in [middle_doc, bottom_doc]:
        for element in src_doc.element.body:
            top_doc.element.body.append(element)

    # Handle headers and footers
    _merge_headers_and_footers(top_doc, middle_doc, bottom_doc)

    # Handle media (images) from all documents
    _merge_media_files([top_file, middle_file, bottom_file], top_doc)

    # Save the final document
    top_doc.save(output_file)

def _merge_headers_and_footers(dest_doc, middle_doc, bottom_doc):
    """
    Copies header/footer content from the top document, appending content from middle and bottom documents if necessary.
    """
    # Only keep headers/footers from the top document (dest_doc)
    dest_sections = dest_doc.sections
    middle_sections = middle_doc.sections
    bottom_sections = bottom_doc.sections

    for i, section in enumerate(dest_sections):
        if i < len(middle_sections):
            _copy_header_footer_content(middle_sections[i].header, section.header)
            _copy_header_footer_content(middle_sections[i].footer, section.footer)

        if i < len(bottom_sections):
            _copy_header_footer_content(bottom_sections[i].header, section.header, append=True)
            _copy_header_footer_content(bottom_sections[i].footer, section.footer, append=True)

def _copy_header_footer_content(src_header_footer, dest_header_footer, append=False):
    """
    Copies content from a source header/footer to a destination header/footer.
    If `append=True`, it appends the content instead of overwriting.
    """
    if not append:
        # Clear the destination header/footer
        for child in list(dest_header_footer._element):
            dest_header_footer._element.remove(child)

    # Copy elements from the source to the destination
    for child in src_header_footer._element:
        dest_header_footer._element.append(child)

def _merge_media_files(doc_files, target_doc):
    """
    Merges media (e.g., images) from all source documents into the target document.
    """
    for doc_file in doc_files:
        # Open the document and copy images using the copy_images function
        source_doc = Document(doc_file)
        copy_images(source_doc, target_doc)

def copy_images(source_doc, target_doc):
    """Copy images from source to target document."""
